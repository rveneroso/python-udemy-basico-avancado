...
"""
'''
O conceito de abstração apresentado nessa aula não é o mesmo que eu conheço de Java. Aqui, não se trata de implementação de classes que não pode ser instanciadas.
Abstração aqui tem a ver com o fato de ocultar os detalhes das regras de negócio sendo executadas. Por exemplo: o método tranferir faz algumas validações e realiza
operações envolvendo as duas contas. Porém, o cliente que faz uso desse método não tem o menor conhecimento do que está sendo feito ali. É nesse sentido que o
termo abstração é apresentado nessa aula.
'''
